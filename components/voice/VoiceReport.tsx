...
import { Mic, StopCircle, Play, Pause, Send, Loader2, Trash2 } from 'lucide-react';
...
export default function VoiceReport({ onSubmit }: VoiceReportProps) {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (SpeechRecognition) {
        ...
        recognition.onerror = (event: any) => {
          // Log come warning, non error, per non spaventare
          if (event.error === 'no-speech') {
             ...
             setRecognitionAvailable(false); // Disabilita per il futuro
          } else {
        ...
        recognition.onend = () => {
          // Se siamo ancora in recording, prova a riavviare solo se era disponibile
  ...
  const startRecording = async () => {
    try {
      ...
      // Prova a far partire il riconoscimento solo se disponibile
  ...
  const handleSubmit = async () => {
    ...
    if (onSubmit) {
      // Se non c'è transcript (perché non supportato), metti un placeholder
    ...
    <div className="bg-black/40 backdrop-blur-md border border-slate-800 rounded-xl p-8 flex flex-col items-center justify-center relative overflow-hidden h-full">
      ...
      <div className="mb-8">
          <div className={clsx(
              ...
              status === 'recording' ? "border-red-500/50 bg-red-500/10 text-red-400 animate-pulse" :
      ...
      <div className="flex items-center gap-8 mb-8 relative z-10">
        {!isRecording && status !== 'completed' ? (
          <motion.button
            ...
            className="w-24 h-24 rounded-full bg-gradient-to-br from-red-500 to-rose-600 flex items-center justify-center shadow-[0_0_50px_rgba(225,29,72,0.4)] border-4 border-red-400/20 group transition-all"
          >
           ...
           <div className="flex gap-4">
              ...
              <motion.button
                ...
                className="px-6 py-4 border border-slate-700 hover:border-red-500 text-slate-400 hover:text-red-500 rounded-lg transition-colors"
              >
