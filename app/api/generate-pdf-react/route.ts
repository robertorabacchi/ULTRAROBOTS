...
import { fetchOfficialAddress } from '@/lib/company-address';

const limitCompanyField = (value?: string | null) => {
  if (!value) return '';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const officialAddress = await fetchOfficialAddress(completeData.cliente.azienda);
    if (officialAddress) {
      if (officialAddress.street) {
        completeData.cliente.indirizzo = officialAddress.street;
      } else if (officialAddress.full) {
        completeData.cliente.indirizzo = officialAddress.full;
      }

      if (officialAddress.cityLine && !completeData.cliente.citta?.trim()) {
        completeData.cliente.citta = officialAddress.cityLine;
      }

      if (officialAddress.full && !completeData.cliente.sede?.trim()) {
        completeData.cliente.sede = officialAddress.full;
      }
    }

    completeData.cliente.azienda = limitCompanyField(completeData.cliente.azienda);
    completeData.cliente.indirizzo = limitCompanyField(
      completeData.cliente.indirizzo || completeData.cliente.sede
    );
    completeData.cliente.sede = limitCompanyField(completeData.cliente.sede);
    completeData.cliente.citta = limitCompanyField(completeData.cliente.citta);
    completeData.cliente.piva = limitCompanyField(completeData.cliente.piva);
    completeData.cliente.telefono = limitCompanyField(completeData.cliente.telefono);
    completeData.cliente.email = limitCompanyField(completeData.cliente.email);
    completeData.cliente.referente = limitCompanyField(completeData.cliente.referente);
